...
from Tensor2DPhysique import Tensor2DPhysique
...
class MaterialSimulationApp(QMainWindow):
    ...
    def __init__(self, mode="2D", switch_callback=None):
        super().__init__()
        self.physique = Tensor2DPhysique()
        self.setWindowTitle(
    ...
    def _calculer_physique(self):
        ...
        donnees, paires, stats = self.physique.calculer_contraintes(blocs)
        aire = stats["aire"]
        XG = stats["XG"]
        YG = stats["YG"]
        Ixx = stats["Ixx"]
        ...
        for i, (bloc, stress) in enumerate(zip(blocs, donnees)):
            _, sym = self.physique.statut_utilisation(stress["utilization"])
            statut, _ = self.physique.statut_utilisation(stress["utilization"])
            taux = stress["utilization"]

            resumes.append(
                f"  Bloc <b>{i + 1}</b> ({bloc['material']}) : "
                f"σ = <b>{stress['sigma_total'] / 1e6:.2f} MPa</b>, "
                f"<b>{taux:.0f}%</b> {sym}")

            lignes_detail += [
                f"<b style='color:#e65100'>── Bloc {i + 1} ({bloc['material']}) ──</b>",
                f"  Poids propre   : {bloc['density'] * bloc['patch'].get_width() * bloc['patch'].get_height() * 9.81:.1f} N",
                f"  Force ext.     : {bloc['ext_force']:.1f} N",
                f"  Pression       : {bloc['pressure'] * bloc['patch'].get_width():.1f} N",
                f"  <b>F axiale total : {stress['F_axial']:.1f} N</b>",
                f"  σ axiale       : {stress['sigma_axial'] / 1e6:.3f} MPa",
                f"  <b>σ max          : {stress['sigma_total'] / 1e6:.3f} MPa</b>",
        ...
        if paires:
            ...
            for (ib, ih, frac) in paires:
                fc = donnees[ih]["F_axial"]
                lignes_contact.append(
                    f"  Bloc {ih + 1} → Bloc {ib + 1} : "
                    f"<b>{fc:.0f} N</b> — recouvrement <b>{frac * 100:.0f}%</b>")
            lignes_contact.append("")

        rapport = (
                entete
                + ["<b style='color:#2e7d32'>══ Contraintes ══</b>"]
                + resumes
        ...
        self.panneau.afficher_cdgr(
            ...
            f"<b style='color:#f9a825'>⊕ Centre de gravité</b><br><br>"
            f"<span style='color:#555'>Position (x, y) en mètres<br>"
            f"<b style='color:#e65100;font-size:13px'>({XG:.2f}, {YG:.2f})</b>"
            f"</div>"
        )
        return donnees, paires
